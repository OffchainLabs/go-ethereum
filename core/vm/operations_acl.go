...
func makeGasSStoreFunc(clearingRefund uint64) gasFunc {
	return func(evm *EVM, contract *Contract, stack *Stack, mem *Memory, memorySize uint64) (multigas.MultiGas, uint64, error) {
		...
		if current == value { // noop (1)
			...
			return multigas.NewEmptyMultiGas(), cost + params.WarmStorageReadCostEIP2929, nil // SLOAD_GAS
		}
		original := evm.StateDB.GetCommittedState(contract.Address(), x.Bytes32())
		if original == current {
			if original == (common.Hash{}) { // create slot (2.1.1)
				return multigas.NewEmptyMultiGas(), cost + params.SstoreSetGasEIP2200, nil
			}
			...
			return multigas.NewEmptyMultiGas(), cost + (params.SstoreResetGasEIP2200 - params.ColdSloadCostEIP2929), nil // write existing slot (2.1.2)
		}
		...
		return multigas.NewEmptyMultiGas(), cost + params.WarmStorageReadCostEIP2929, nil // dirty update (2.2)
	}
}
...
func gasSLoadEIP2929(evm *EVM, contract *Contract, stack *Stack, mem *Memory, memorySize uint64) (multigas.MultiGas, uint64, error) {
	...
	if _, slotPresent := evm.StateDB.SlotInAccessList(contract.Address(), slot); !slotPresent {
		...
		evm.StateDB.AddSlotToAccessList(contract.Address(), slot)
		return multigas.NewEmptyMultiGas(), params.ColdSloadCostEIP2929, nil
	}
	return multigas.NewEmptyMultiGas(), params.WarmStorageReadCostEIP2929, nil
}
...
func gasEip2929AccountCheck(evm *EVM, contract *Contract, stack *Stack, mem *Memory, memorySize uint64) (multigas.MultiGas, uint64, error) {
	...
	if !evm.StateDB.AddressInAccessList(addr) {
		...
		return multigas.NewEmptyMultiGas(), params.ColdAccountAccessCostEIP2929 - params.WarmStorageReadCostEIP2929, nil
	}
	...
	return func(evm *EVM, contract *Contract, stack *Stack, mem *Memory, memorySize uint64) (multigas.MultiGas, uint64, error) {
		...
		if !warmAccess {
			...
			if !contract.UseGas(coldCost, evm.Config.Tracer, tracing.GasChangeCallStorageColdAccess) {
				return multigas.NewEmptyMultiGas(), 0, ErrOutOfGas
			}
		}
...
func makeCallVariantGasCallEIP7702(oldCalculator gasFunc) gasFunc {
	return func(evm *EVM, contract *Contract, stack *Stack, mem *Memory, memorySize uint64) (multigas.MultiGas, uint64, error) {
		...
		return multigas.NewEmptyMultiGas(), total, nil
	}
}
