...
func memoryGasCost(mem *Memory, newMemSize uint64) (multigas.MultiGas, uint64, error) {
	...
	if newMemSize > uint64(mem.Len()) {
		...
		return multigas.NewEmptyMultiGas(), fee, nil
	}
	return multigas.NewEmptyMultiGas(), 0, nil
}
...
func gasSStore(evm *EVM, contract *Contract, stack *Stack, mem *Memory, memorySize uint64) (multigas.MultiGas, uint64, error) {
	...
	if evm.chainRules.IsPetersburg || !evm.chainRules.IsConstantinople {
		...
		case current == (common.Hash{}) && y.Sign() != 0: // 0 => non 0
			return multigas.NewEmptyMultiGas(), params.SstoreSetGas, nil
		case current != (common.Hash{}) && y.Sign() == 0: // non 0 => 0
			evm.StateDB.AddRefund(params.SstoreRefundGas)
			return multigas.NewEmptyMultiGas(), params.SstoreClearGas, nil
		default: // non 0 => non 0 (or 0 => 0)
			return multigas.NewEmptyMultiGas(), params.SstoreResetGas, nil
		}
	}
	...
	if current == value { // noop (1)
		return multigas.NewEmptyMultiGas(), params.NetSstoreNoopGas, nil
	}
	original := evm.StateDB.GetCommittedState(contract.Address(), x.Bytes32())
	if original == current {
		if original == (common.Hash{}) { // create slot (2.1.1)
			return multigas.NewEmptyMultiGas(), params.NetSstoreInitGas, nil
		}
		if value == (common.Hash{}) { // delete slot (2.1.2b)
			evm.StateDB.AddRefund(params.NetSstoreClearRefund)
		}
		return multigas.NewEmptyMultiGas(), params.NetSstoreCleanGas, nil // write existing slot (2.1.2)
	}
	...
	return multigas.NewEmptyMultiGas(), params.NetSstoreDirtyGas, nil
}
...
func gasSStoreEIP2200(evm *EVM, contract *Contract, stack *Stack, mem *Memory, memorySize uint64) (multigas.MultiGas, uint64, error) {
	...
	if contract.Gas <= params.SstoreSentryGasEIP2200 {
		return multigas.NewEmptyMultiGas(), 0, errors.New("not enough gas for reentrancy sentry")
	}
	...
	if current == value { // noop (1)
		return multigas.NewEmptyMultiGas(), params.SloadGasEIP2200, nil
	}
	original := evm.StateDB.GetCommittedState(contract.Address(), x.Bytes32())
	if original == current {
		if original == (common.Hash{}) { // create slot (2.1.1)
			return multigas.NewEmptyMultiGas(), params.SstoreSetGasEIP2200, nil
		}
		if value == (common.Hash{}) { // delete slot (2.1.2b)
			evm.StateDB.AddRefund(params.SstoreClearsScheduleRefundEIP2200)
		}
		return multigas.NewEmptyMultiGas(), params.SstoreResetGasEIP2200, nil // write existing slot (2.1.2)
	}
	if original != (common.Hash{}) {
		if current == (common.Hash{}) { // recreate slot (2.2.1.1)
	...
	return multigas.NewEmptyMultiGas(), params.SloadGasEIP2200, nil // dirty update (2.2)
}
...
func gasCreateEip3860(evm *EVM, contract *Contract, stack *Stack, mem *Memory, memorySize uint64) (multigas.MultiGas, uint64, error) {
	...
	if size > evm.chainConfig.MaxInitCodeSize() {
		return multigas.NewEmptyMultiGas(), 0, fmt.Errorf("%w: size %d", ErrMaxInitCodeSizeExceeded, size)
	}
