...
func execute(ctx context.Context, call *core.Message, opts *Options, gasLimit uint64) (bool, *core.ExecutionResult, error) {
	...
	// other non-fixable conditions. Estimate does not surface filtered records.
...
// call invocation. When the transaction is filtered it returns the matching
// records alongside state.ErrArbTxFilter; otherwise the returned slice is nil.
func run(ctx context.Context, call *core.Message, opts *Options) (*core.ExecutionResult, []filter.FilteredAddressRecord, error) {
