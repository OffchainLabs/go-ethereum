...
func setupTestBlockchain(t *testing.T, genesis *core.Genesis, tx *types.Transaction, useBeacon bool) (*core.BlockChain, *types.Block, *state.StateDB) {
	...
	blockchain, err := core.NewBlockChain(db, nil, genesis, engine, nil)
	if err != nil {
...
func TestSelfdestructStateTracer(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			msg, err := core.TransactionToMessage(tx, signer, nil, core.NewMessageReplayContext())
			if err != nil {
			...
			usedGas := uint64(0)
			_, _, err = core.ApplyTransactionWithEVM(msg, new(core.GasPool).AddGas(tx.Gas()), statedb, block.Number(), block.Hash(), block.Time(), tx, &usedGas, evm, nil)
			if err != nil {
